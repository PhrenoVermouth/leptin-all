...
ggsave("Volcano_w4.pdf",dpi=300,width = 5,height = 4)
...
ggsave("Volcano_w8.pdf",dpi=300,width = 5,height = 4)
...
ggsave("Volcano_w16.pdf",dpi=300,width = 5,height = 4)
...
ggsave("Volcano_w32.pdf",dpi=300,width = 5,height = 4)
...
ggsave("Volcano_w48.pdf",dpi=300,width = 5,height = 4)
...
############ Seperated heatmap. 
inflam_normat <- mean_normat[which(rownames(mean_normat) %in% inflam_geneset),]
...
t.test(inflam_normat[,2],inflam_normat[,7],paired = T)
t.test(fat_normat[,2],fat_normat[,7],paired = T)

wilcox.test(fat_normat[,5],fat_normat[,10])
...
david2gp <- function(csv,title_def) {
  csv <- read.csv(csv,header = F,col.names = c("Desc.","Pvalue","LogPvalue"))
  p <- ggplot(csv) + geom_bar(aes(x=reorder(Desc.,LogPvalue),y=LogPvalue),stat = "identity",fill="Aquamarine1")+coord_flip()
  p <- p + theme(panel.grid.major =element_blank(),
    ...
    labs(title=title_def)+ theme(plot.title = element_text(hjust = 0.5)) 
  return(p)
}
david2gp("Fig1_Cluster1(W16,32).csv","Cluster2")
